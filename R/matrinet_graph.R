...
#' @title Initialize matrigraph object
#' @description Create the initial skeleton of matrigraph object for storing network data.
#' @param datalist The output from matrinet_init() function or any list of group (cancer) specific data-frames containing node covariates, e.g., gene or protein expression data. Extracted automatically from the dataframe by the column names of prior_topology object.
...
#' @import dplyr
#' @examples print(c("matrinet_graph(datalist, prior_topology)"))
#' @export


matrinet_graph <- function(datalist, prior_topology, add_node_df = NULL, add_edge_df = NULL){
  #### STEP 1 #####
  network_object <- lapply(datalist, function(x){
